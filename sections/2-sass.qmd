...
<center><div class="custom-subtitle2">Next, create a `styles.scss` file (name it however you want), link to it in `_quarto.yml` (this applies your styles to your Quarto site), and define your Sass variables.</div></center>
...
format:
  html:
    theme: styles.scss # I've also removed the `css: styles.css` option that was included by default
    toc: true
    page-layout: full
```
...
Quarto provides a [list of Sass Variables](https://quarto.org/docs/output-formats/html-themes.html#sass-variables) that can be specified within `.scss` files. We can assign our newly minted color variables *as values* to specific Quarto Sass variables to easily update things like the background color, navbar & footer colors, hyperlink color, and more.
