...
[**1.**]{.teal-text} Create a `.scss` file in your repo's root directory using the `touch` command in the Terminal (alternatively, use **New Blank File** > **Text File** in the Files pane):
...
[**3.**]{.teal-text} Apply your `styles.scss` file to your website using the `theme` option (*Note:* if you have your preview running, you'll likely see an *Bad resource ID* error appear until we add more content to our `styles.scss` file): 
