---
format: 
  revealjs:
    slide-number: true
    preview-links: auto
    code-link: true
    highlight-style: a11y
    chalkboard: true
    #scrollable: true
    theme: 
      - meds_styles.scss
---

## {#title-slide data-menu-title="Title Slide" background="images/title_background.png"} 

[Customizing Quarto Websites]{.custom-title}

[*Make your website stand out using `r fontawesome::fa("sass", fill = "#d2e3f3", a11y = "sem")` & `r fontawesome::fa("css3-alt", fill = "#d2e3f3", a11y = "sem")`*]{.custom-subtitle}

<br> 
<br>

:::: {.columns} 

::: {.column width="50%"}
[Sam Csik |]{.custom-subtitle2}<br>
[Data Training Coordinator]{.custom-subtitle2}<br>
[*National Center for Ecological Analysis & Synthesis*]{.custom-subtitle3}
:::

::: {.column width="50%"}
[Master of Environmental Data Science |]{.custom-subtitle2}<br>
[Fall 2022]{.custom-subtitle2}<br>
[*Bren School of Environmental Science & Management*]{.custom-subtitle3}
:::

::::

<br>

<center><span style='font-size: 50%; color: #d2e3f3'>Slides & source code available on [`r fontawesome::fa("github", fill = "#d2e3f3", a11y = "sem")`](https://github.com/UCSB-MEDS/customizing-quarto-websites)</span></center>

---

{{< include sections/1-prerequisites.qmd >}}

---

{{< include sections/yaml.qmd >}}

---

{{< include sections/2-intro_html_css.qmd >}}

---

{{< include sections/3-intro_html.qmd >}}

---

{{< include sections/4-intro_css.qmd >}}

---

{{< include sections/5-css_quarto.qmd >}}

---

## Inspecting website

---

## Write some CSS (in styles.css)
(note to make branch)

---

## Quarto vs CSS syntax
(I use a mix of both)

---

## Before we get too far...Enter Sass

---

## Create .scss file & start editing

---

## Look for inspiration (and code) elsewhere

---

## Other fun things to change:

- logo
- favicon
- footer
- link cv

---

## {#TheEnd data-menu-title="~End Slide~"}

<center><div class="slide-title">Now get to customizing!</div></center>

<br>

<center><img src="images/hacker.gif" alt="" width="700"/></center>

::: aside
<center>This is a Quarto Presentation. To learn more, visit <https://quarto.org>.</center>

<center><span style = 'font-size: 60%;'>*If you see mistakes or want to suggest changes, please create an [issue](https://github.com/UCSB-MEDS/customizing-quarto-websites/issues) on the source repository.*</span></center>
:::
